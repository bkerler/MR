#Pluginname="Google Maps Cache (Android,FS)"
#Category="Extraction"
#(c) B.Kerler 2017
#Type=FS
...
def main():
    ...
    if (len(tiles)>0):
        ...
        with open(extracttodir+"/coordinates.csv","wb") as cw:
            cw.write(b"Filename;Latitude;Longitude;Date\n")
            for item in tiles:
                ctx.gui_setMainProgressBar(int(i/len(tiles)))
                ctx.fs_file_extract(item,extracttodir+"/tmp")
                ctx.gui_setMainLabel("Status: Converting images and extracting coordinates...")
                i+=1
                with open(extracttodir+"/tmp","rb") as rb:
                    ...
                    while True:
                        ...
                        if (timestamp!=0):
                            try:
                                date = datetime.datetime.utcfromtimestamp(timestamp).strftime("%d.%m.%Y %H:%M:%S")
                            except:
                                print("Setting timestamp failed on "+filename)
                        rb.seek(0x10,1)
                        buf=rb.read(4)
                        if not buf:
                             break
                        ...
                        if (magic == 0x54415244): #DRAT
                            ...
                            if (struct.unpack("I",bytearray(jpg)[0:4])[0]==0x474E5089):
                                extension=".png"
                            else:
                                extension=".jpg"
                            filename=extracttodir+"/img"+item[item.rfind(".")+1:]+"_"+str(counter)+"_lat_"+latstr+"_lon_"+lonstr+extension
                            with open(filename,"wb") as wf:
                                wf.write(jpg)
                                cw.write(bytearray(item+";"+latstr+";"+lonstr+";"+date+"\n","utf-8"))
                                counter+=1
                            if timestamp!=0:
                                try:
